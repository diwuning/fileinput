...
/**
 * Created by wangchm on 2016/11/29 0029.
 */
...
//        setBackgroundColor(getResources().getColor(R.color.colorPrimary));
    ...
    //定义一个LayoutParams用它来控制子view的位置
    private LayoutParams lp;
    public void init(){
        //初始化显示的图片
        drawables = new Drawable[3];
        Drawable red = getResources().getDrawable(R.drawable.icon_zan_pressed);
        Drawable yellow = getResources().getDrawable(R.drawable.icon_zan_nor_1);
        Drawable blue = getResources().getDrawable(R.drawable.icon_zan_nor);
        drawables[0] = red;
        drawables[1] = yellow;
        drawables[2] = blue;
        //获取图的宽高，用于后面的计算
        //注意 这里用的3张图片的大小都是一样的，只取了一个的大小
        dHeight = red.getIntrinsicHeight();
        dWidth = red.getIntrinsicWidth();

        //底部水平居中
        lp = new LayoutParams(dWidth,dHeight);
        lp.addRule(CENTER_HORIZONTAL,TRUE);//这里的TRUE要注意，不是true
